...
The process of securing an operating system by reducing its attack surface, removing unnecessary components, and applying strong access controls
...
Patching removes known vulnerabilities. apt update refreshes package metadata, apt upgrade installs fixes. Reboot checks ensure new kernels are active. 
...
User Account Security is the process of protecting Linux user accounts from unauthorized access, privilege escalation, and misuse.
 
```bash
# List all users
cut -d: -f1 /etc/passwd

#Inventory all accounts on the system.
...
#An empty password may allow unauthorized access depending on authentication settings.
...
#Unexpected login-enabled accounts
```
...
## 3. SSH Hardening
SSH (Secure Shell) is a secure network protocol used to log into remote servers, run commands, and transfer files over an unencrypted network
Common uses:
...
Key-based authentication, disabling root login, limiting authentication attempts, and restricting users significantly reduce risk. 
 
### Create a non-root sudo user first!
Never log in directly as root.
Better accountability
Reduced risk of root compromise
Better audit trails
...
### Edit SSH config
validates configuration syntax before restart
```bash
...
Port 2222: Changes the SSH connection port from default 22 to 2222 to reduce automated bot attacks.

PermitRootLogin no: Disables direct login as the root user to force administrative access through non-root accounts.

PasswordAuthentication no: Blocks password-based logins, requiring key-based authentication instead.

PubkeyAuthentication yes: Enables authentication using SSH key pairs for secure login.

PermitEmptyPasswords no: Rejects login attempts from accounts that do not have a password set.

X11Forwarding no: Disables remote graphical interface forwarding to minimize security vulnerabilities.

MaxAuthTries 3: Limits failed authentication attempts per connection to 3 before disconnecting.

ClientAliveInterval 300: Sets the server to send a check-in ping to idle clients every 300 seconds (5 minutes).

ClientAliveCountMax 2: Disconnects the client if it fails to respond to 2 consecutive ping checks (10 minutes total idle timeout).

LoginGraceTime 30: Gives users 30 seconds to complete authentication before dropping the unauthenticated connection.

AllowUsers deploy: Restricts SSH access exclusively to the specified user account (deploy).

Protocol 2: Forces the use of SSH Protocol version 2 (deprecated in modern OpenSSH as Protocol 1 is fully removed).
...
A firewall hardens a Linux server by allowing only required network traffic and blocking all unauthorized connections by defaul
...
```
sudo apt install ufw -y: Installs the UFW firewall package automatically without asking for manual confirmation.

sudo ufw default deny incoming: Blocks all incoming network traffic by default to secure open ports from unauthorized access.

sudo ufw default allow outgoing: Permits all outbound connections initiated by the server, allowing updates and external API calls.

sudo ufw allow 2222/tcp: Opens TCP port 2222 to permit remote SSH logins on your custom SSH port.

sudo ufw allow 80/tcp: Opens TCP port 80 to serve unencrypted web traffic over standard HTTP.

sudo ufw allow 443/tcp: Opens TCP port 443 to serve encrypted web traffic over secure HTTPS.

sudo ufw enable: Activates the firewall and enforces all configured rules across system reboots.

sudo ufw status verbose: Displays a detailed list of all active firewall rules, default policies, and network status.
...
Linux utility used to configure and manage the kernel's firewall rules.

-L

List all firewall rules in each chain (INPUT, OUTPUT, FORWARD).
...
Numeric output. Displays IP addresses and ports as numbers without DNS lookups, making the output faster and more accurate. 
---
 
## 5. Fail2Ban (Brute-Force Protection)
 
Automatically bans IPs that show malicious behavior.
Fail2Ban automatically detects repeated failed login attempts and temporarily blocks the attacker's IP address to protect services such as SSH from brute-force attacks.
