...
These are Linux device names used to identify disks, partitions, and special storage devices.
 
In Linux, every storage device is treated as a file under `/dev`.
 
| Device pattern | Meaning |
|---|---|
| `/dev/sda`, `/dev/sdb` | SATA / SCSI / USB disks | Physical disks |
| `/dev/sda1`, `/dev/sda2` | Partitions of /dev/sda; sda1 is the first partition, sda2 the second|
| `/dev/nvme0n1` | NVMe SSD |
| `/dev/nvme0n1p1` | First partition on NVMe disk |p1 identify the partition
| `/dev/mapper/...` | LVM logical volumes or LUKS-encrypted devices |Virtual devices created by device-mapper
| `/dev/loop0` | Loopback device (e.g., a mounted ISO) |Loop device, which lets a file such as an ISO or disk image be treated like a disk.
...
The typical layout is: **physical disk → partition → filesystem → mount point**.
        ...
        ┌────┼────┐           ┌────┼────┐
        │    │      │          	 │    │       │
      	sda1  sda2 sda3      	 p1   p2   p3
 
---
 
## 2. Viewing Disks and Partitions

Block Device - storage device that Linux can read/write data to, such as a hard disk, SSD, USB drive, or partition.
